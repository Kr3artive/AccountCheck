...
const App = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/listbanks")
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const response = await axios.post("http://localhost:8000/getname", payload);
      console.log(response.data);
      setAccountName(response.data.data.account_name || "Account name not found"); 
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      <div className="bg-white shadow-md rounded-lg p-6">
        <h2 className="text-xl font-semibold mb-4 text-center">CHECK FOR ACCOUNT NUMBER</h2>
